...
- The CLI **defaults to the PROD backend** (`https://easybooks.jackyzhang.app`, the immicore eb-plugin via the eb frontend nginx `/api` proxy); the legacy Node `http://localhost:8080` is no longer the default. Override to test (`https://easybooks-test.jackyzhang.app`) or LAN (`http://192.168.1.98:8310`) via `--base-url`. Because the default is production, creating or sending an invoice is a production write and requires an approval artifact (see `easybooks-capabilities` §G). `invoice send` against production emails a real client — if you'd be hitting production without an approval artifact, **stop** and tell the user.
- Creating and sending invoices require a **read_write** API key. If the CLI returns a scope/permission error, the user's key is read-only — tell them to create a Read & write key in the EasyBooks web app (Settings → API Keys).
