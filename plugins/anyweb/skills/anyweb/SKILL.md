...
Playbooks: [connect](references/connect.md), [repair claims](references/repair.md),
and [Tell Jacky](references/tell-jacky.md).
...
Runtime data stays in `~/.jackyzhang.app/anyweb/`; the only durable credential
is the shared `~/.jackyzhang.app/token/user.json`. Never expose repair refs as
website content, and never treat `repair_available` as permission to submit,
