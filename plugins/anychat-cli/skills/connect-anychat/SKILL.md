...
description: >-
  ...
  AnyChat CLI (`anychat login`). Run this before any other AnyChat skill
  when ~/.anychat/config.json is missing.
when_to_use: |-
  Trigger phrases:
...
`login` tries Portal HTTPS entitlement first (`POST …/api/v1/anychat/entitlement`).  
...
- Never put the token in skill examples as a real value.
