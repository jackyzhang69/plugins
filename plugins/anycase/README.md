...
- 🤖 **Agent-First Architecture**: Standardized `anycase` interface for Codex, Claude Code, and Gemini.
...
### 1. Installation
Add `anycase` to your `PATH` or install the plugin directly:
```bash
...
```bash
anycase clb --test ielts -l 8.0 -r 7.0 -w 7.0 -s 7.5
```

#### ⚖️ Search Case Law Precedents
```bash
anycase caselaw "study permit dual intent section 22(2)" --top 3
anycase caselaw "procedural fairness extrinsic evidence" --top 3
```

#### 📚 Search Official IRCC Help Centre Q&As
```bash
anycase policy "maintained status travel outside canada" --top 3
anycase policy "学签 续签" --lang zh --top 3
```

#### 📖 Search IRCC Program Delivery Instructions
```bash
anycase manual "study permit dual intent" --mode hybrid --top 5
anycase coverage
```

#### 🤖 Agent Tool Router (JSON Interface)
```bash
anycase query --action caselaw --input "refusal financial sufficiency" --top 2
anycase query --action manual --input "study permit dual intent" --top 5
...
├── bin/
│   └── anycase                # Standalone native Rust binary executable
├── src/
...
├── skills/
│   └── anycase/               # Product router skill and playbooks
├── scripts/
