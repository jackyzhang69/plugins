...
human hears. Send the current inspect and assemble status; follow the next
already-allowed AnyDoc action that comes back.

Not Tell Jacky. Do not send document bytes, excerpts, packing lists, tokens, or
paths.

If AnyDoc is not connected, follow [connect](connect.md) first.

## Confirm first

Show the human: Jacky's assistant will look at this machine's AnyDoc inspect
and assemble status for about two hours, and you will keep talking to them as
usual. Wait for explicit agreement. Then:

```bash
"$ANYDOC_BIN" pair join --code <CODE> --user-confirmed --json
```
...
```bash
"$ANYDOC_BIN" pair snapshot --envelope-json - --json
"$ANYDOC_BIN" pair inbox --json
```

Unread turns:

- `ask_say` — speak `text` to the human in product language
- `ask_human` — the human-action already required by AnyDoc
- `ask_run` — only `doctor` or `whoami`, without remote arguments
- `ask_continue` — obtain the current continuation from your own local AnyDoc
...
```bash
"$ANYDOC_BIN" pair read --message-id <ID> --json
```

When you ran something, send the outcome:

```bash
"$ANYDOC_BIN" pair result --ok --json
"$ANYDOC_BIN" pair result --error "short product reason" --json
```
...
action report only the bounded result and minimal product status. Customer
files stay on this computer.

## End

```bash
"$ANYDOC_BIN" pair close --json
```

The session expires in two hours, or either side closes it. Then go back to
ordinary AnyDoc. Offer Tell Jacky only if the product envelope says so.
