...
3. **Login follows the chosen authority.** Inspect and an explicitly user-provided `manual_plan` stay offline. Resolving or changing a saved private model requires [connect](connect.md). If `token/user.json` already exists from any official plugin, do not ask them to log in again.
...
| `compress_to_explicit_budget` | Unencrypted, unsigned, non-form PDFs only. Requires `max_bytes`. Optional `clarity_tier`: `clarity_priority` (default), `balanced`, or `max_shrink`. No unlimited tier. Rewrites embedded rasters using the longest edge (the longer of width and height) plus JPEG quality, and never crosses that tier's floor (clarity_priority: longest edge ≥ 1280 / JPEG ≥ 70; balanced: ≥ 768 / ≥ 55; max_shrink: ≥ 512 / ≥ 40). Already under the budget is copied as-is. Before approval, read `expected_quality_loss` and say that worst case. If the floor is reached but the size is met, `delivery_status` is `at_clarity_floor` (`clarity_floor_reached`). If the floor file is still over the budget, deliver it with `delivery_status` `over_budget_at_floor` (`compress_floor_over_budget`) and do not split. |
...
`constraints.max_file_bytes` is required, and AnyDoc never supplies a number of its own. Cite the byte cap the portal or guide states, or write `null` to record that no cap is cited; a plan without the key is refused as `missing_max_file_bytes`. With a cited cap, a result over it fails unless the overage came from `compress_to_explicit_budget` (then it is a warning, not a hard stop). With `null`, no output size limit is checked and every preview says so. Compression defaults to `clarity_tier: clarity_priority`. Before the user approves, show `expected_quality_loss` (worst-case longest edge and JPEG quality for the selected tier). Never cross that floor to chase size, and never offer a no-floor mode. After assemble or verify, read `delivery_status`. `complete` means the size was met above the floor. `at_clarity_floor` means the size was met only at the floor (`clarity_floor_reached`) — say the floor numbers. `over_budget_at_floor` means the floor file is still too big (`compress_floor_over_budget`): the file is delivered, but this is not an ordinary ready-to-submit result. The user chooses whether to split it, drop content, lower the tier, or accept the portal's size risk. `over_budget` means a cited cap was still missed without stopping on the floor; same decision. Do not report a floor hit or an over-budget delivery as quiet success.
...
| File over a portal size | Compression runs only when the packing list names an explicit byte budget and a clarity tier (default: clarity priority). Before it runs, say the worst-case longest-edge and JPEG-quality floor. AnyDoc will not go below that floor to hit the size. If the floor file is still too big, you get that file and a status that is not a normal success — you decide whether to split it, drop content, pick a smaller tier, or accept the portal's risk. |
