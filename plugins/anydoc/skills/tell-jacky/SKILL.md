...
name: tell-jacky
description: >-
  Submit a feature request, bug report, or knowledge tip for AnyDoc ("Tell
  Jacky") through the AnyDoc CLI. Always show a draft and get explicit user
  confirmation before sending. Requires connect-anydoc when no user.json exists.
when_to_use: |-
  Trigger phrases:
...
If `anydoc doctor --json` reports `credential.configured=false`, load **connect-anydoc** first. If `user.json` already exists, do not ask for another token.
