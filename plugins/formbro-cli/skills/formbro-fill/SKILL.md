...
- ✅ Output is JSON with a `files` array (each with `path`, `form`, `size_bytes`) plus an `engine` marker. `--engine auto` is the default: local engine is used for standard bundled PR/TR forms; backend fallback is reserved for real runtime failures or genuinely unsupported future forms.
...
The `engine` field is telemetry, not a task failure. In `auto` mode, current bundled standard PR/TR PDFs should normally report `engine: "local"`; `engine: "backend"` means the CLI used its exception path after local runtime failure or a genuinely unsupported future form. Agents should treat `ok: true` and the `files[]` array as the success contract, but report repeated backend fallbacks as plugin/runtime readiness issues.
...
`0008, 0104, 1294, 1295, 1344, 5257, 5406, 5476, 5532, 5562, 5645, 5669, 5708, 5709, 5710` — standard bundled PR/TR/LMIA PDFs fill locally. IMM5669 uses the AcroForm Setvalue+Bun path.
...
|---|---|
| `export pdf` | Requires explicit `--program-key <key>` AND was TR-route-only in cli; agents that called it for PR cases got 404. `fill` auto-detects category and routes to the right backend endpoint. |
