...
8. **Cache cleanup boundary:** old cache/staging directories and stale `.codex-marketplace-install.json` metadata are artifacts. Do not edit manifests to match a stale path. After successful reinstall and doctor verification, stale version directories or local marketplace metadata can be moved aside, but a running thread may still retain old skill text until a new thread is created.
