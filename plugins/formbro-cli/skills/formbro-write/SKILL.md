...
when_to_use: |-
  Trigger phrases (writes / mutations / pre-submit checks):
    ...
    - "import / create a case from this file / user uploaded Excel/PDF/DOCX"
...
All commands shell out to the bundled `formbro` CLI. Resolve it once via `formbro-capabilities/SKILL.md` §B and invoke that exact path; do not rely on ambient `PATH`.
