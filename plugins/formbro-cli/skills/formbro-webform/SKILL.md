...
3. **Use standalone `preflight` only for inspection/debugging:** run it when the user asks "is this ready?" or when a previous `start` returned a structured blocking error. Do not add a mandatory extra `preflight` call in front of every `start`; that duplicates work.
